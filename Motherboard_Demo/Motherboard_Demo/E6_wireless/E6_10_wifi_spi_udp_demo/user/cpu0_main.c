...
//
// 7.本例程是 UDP Client 例程 模块会被配置为 UDP Client 需要连接到局域网内的 其他UDP节点 才能进行通信
//   目标连接的 UDP Server 的 IP 与端口默认使用 zf_device_wifi_spi.h 中 WIFI_SPI_TARGET_IP 与 WIFI_SPI_TARGET_PORT 定义
//   实际测试需要根据现场 UDP Server 的实际 IP 地址与端口设置
//
// 8.当本机设备主动连接到 UDP Server （例如电脑使用网络调试助手进入 UDP Server 模式 然后本机连接到电脑的 IP 与端口）
...
//
// 问题3：一直显示 Connect UDP Client error, try again. 信息
//      检查默认使用的 UDP Client IP 地址（zf_device_wifi_spi.h 中 WIFI_SPI_TARGET_IP）是否与测试使用的 UDP Client IP 一致
//      检查默认使用的 UDP Client 通信端口（zf_device_wifi_spi.h 中 WIFI_SPI_TARGET_PORT）是否与测试使用的 UDP Client 通信端口一致
//
// 问题4：一直显示 wifi set failed 或者 get module information failed 信息
