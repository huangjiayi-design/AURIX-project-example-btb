...
//
// 8.当本机设备主动连接到 TCP Server （例如电脑使用逐飞多功能上位机进入 TCP Server 模式 然后本机连接到电脑的 IP 与端口）
//   本例程会采集凌瞳图像并发送到滴答客上位机
//
// 9.默认情况下滴答客显示摄像头的图像帧率可以达到50帧，如果无线网络比较复杂例如附近有较多的WIFI热点，可能会导致显示帧率较低
//
//
...
int core0_main(void)
{
    ...
    // 如果要发送图像信息，则务必调用detector_camera_information_config函数进行必要的参数设置
    // 如果需要发送边线则还需调用detector_camera_boundary_config函数设置边线的信息

#if(0 == INCLUDE_BOUNDARY_TYPE)
    // 发送凌瞳图像信息(仅包含原始图像信息)
    seekfree_assistant_camera_information_config(SEEKFREE_ASSISTANT_SCC8660, image_copy[0], OV7725_W, OV7725_H);


#elif(1 == INCLUDE_BOUNDARY_TYPE)
    // 发送凌瞳图像信息(并且包含三条边界信息，边界信息只含有横轴坐标，纵轴坐标由图像高度得到，意味着每个边界在一行中只会有一个点)
    // 对边界数组写入数据
    for(int i = 0; i < SCC8660_H; i++)
    {
        x1_boundary[i] = (uint8)(70 - (70 - 20) * i / SCC8660_H);
        x2_boundary[i] = SCC8660_W / 2;
        x3_boundary[i] = (uint8)(118 + (168 - 118) * i / SCC8660_H);
    }
    detector_camera_information_config(DETECTOR_SCC8660, image_copy[0], SCC8660_W, SCC8660_H);
    detector_camera_boundary_config(X_BOUNDARY, SCC8660_H, x1_boundary, x2_boundary, x3_boundary, NULL, NULL ,NULL);


#elif(2 == INCLUDE_BOUNDARY_TYPE)
    // 发送凌瞳图像信息(并且包含三条边界信息，边界信息只含有纵轴坐标，横轴坐标由图像宽度得到，意味着每个边界在一列中只会有一个点)
    // 通常很少有这样的使用需求
    // 对边界数组写入数据
    for(int i = 0; i < SCC8660_W; i++)
    {
        ...
        y3_boundary[i] = (uint8)((SCC8660_W - i) * SCC8660_H / SCC8660_W);
    }
    detector_camera_information_config(DETECTOR_SCC8660, image_copy[0], SCC8660_W, SCC8660_H);
    detector_camera_boundary_config(Y_BOUNDARY, SCC8660_W, NULL, NULL ,NULL, y1_boundary, y2_boundary, y3_boundary);


#elif(3 == INCLUDE_BOUNDARY_TYPE)
    // 发送凌瞳图像信息(并且包含三条边界信息，边界信息含有横纵轴坐标)
    // 这样的方式可以实现对于有回弯的边界显示
    int j = 0;
    for(int i = SCC8660_H - 1; i >= SCC8660_H / 2; i--)
    {
        // 直线部分
        xy_x1_boundary[j] = 34;
    ...
    for(int i = SCC8660_H / 2 - 1; i >= 0; i--)
    {
        // 直线连接弯道部分
        xy_x1_boundary[j] = (uint8)(34 + (SCC8660_H / 2 - i) * (SCC8660_W / 2 - 34) / (SCC8660_H / 2));
        xy_y1_boundary[j] = (uint8)i;

        xy_x2_boundary[j] = (uint8)(47 + (SCC8660_H / 2 - i) * (SCC8660_W / 2 - 47) / (SCC8660_H / 2));
        xy_y2_boundary[j] = (uint8)(15 + i * 3 / 4);

        xy_x3_boundary[j] = (uint8)(60 + (SCC8660_H / 2 - i) * (SCC8660_W / 2 - 60) / (SCC8660_H / 2));
        xy_y3_boundary[j] = (uint8)(30 + i / 2);
        j++;
    }

    for(int i = 0; i < SCC8660_H / 2; i++)
    {
        // 回弯部分
        xy_x1_boundary[j] = (uint8)(SCC8660_W / 2 + i * (138 - SCC8660_W / 2) / (SCC8660_H / 2));
        xy_y1_boundary[j] = (uint8)i;

        xy_x2_boundary[j] = (uint8)(SCC8660_W / 2 + i * (133 - SCC8660_W / 2) / (SCC8660_H / 2));
        xy_y2_boundary[j] = (uint8)(15 + i * 3 / 4);

        xy_x3_boundary[j] = (uint8)(SCC8660_W / 2 + i * (128 - SCC8660_W / 2) / (SCC8660_H / 2));
        xy_y3_boundary[j] = (uint8)(30 + i / 2);
        j++;
    }
    detector_camera_information_config(DETECTOR_SCC8660, image_copy[0], SCC8660_W, SCC8660_H);
    detector_camera_boundary_config(XY_BOUNDARY, BOUNDARY_NUM, xy_x1_boundary, xy_x2_boundary, xy_x3_boundary, xy_y1_boundary, xy_y2_boundary, xy_y3_boundary);
    ...
    {
        x1_boundary[i] = (uint8)(70 - (70 - 20) * i / SCC8660_H);
        x2_boundary[i] = (uint8)(SCC8660_W / 2);
        x3_boundary[i] = (uint8)(118 + (168 - 118) * i / SCC8660_H);
    }
    detector_camera_information_config(DETECTOR_SCC8660, NULL, SCC8660_W, SCC8660_H);
    detector_camera_boundary_config(X_BOUNDARY, SCC8660_H, x1_boundary, x2_boundary, x3_boundary, NULL, NULL ,NULL);
