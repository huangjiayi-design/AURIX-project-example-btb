...
// 接入 IMU660RX
//      模块管脚            单片机管脚
//      SCL/SPC             查看 zf_device_imu660rx.h 中 IMU660RX_SPC_PIN 宏定义 默认 P20_11
//      SDA/DSI             查看 zf_device_imu660rx.h 中 IMU660RX_SDI_PIN 宏定义 默认 P20_14
//      SA0/SDO             查看 zf_device_imu660rx.h 中 IMU660RX_SDO_PIN 宏定义 默认 P20_12
//      CS                  查看 zf_device_imu660rx.h 中 IMU660RX_CS_PIN  宏定义 默认 P20_13
//      GND                 电源地 GND
//      3V3                 电源 3V3
...
//      IMU660RX acc data: x-..., y-..., z-...
//      IMU660RX gyro data: x-..., y-..., z-...
// 5.移动旋转 IMU660RX 就会看到数值变化
...
int core0_main(void)
{
    ...
    gpio_init(LED1, GPO, GPIO_HIGH, GPO_PUSH_PULL);                             // 初始化 LED1 输出 默认高电平 推挽输出模式
     ...
     {
        if(imu660rx_init())
            printf("\r\n IMU660RX init error.");                                 // IMU660RX 初始化失败
        else
            break;
    ...
    while (TRUE)
    {
        ...
        printf("\r\n IMU660RX acc data:  x=%5d, y=%5d, z=%5d\r\n", imu660rx_acc_x,  imu660rx_acc_y,  imu660rx_acc_z);
        printf("\r\n IMU660RX gyro data: x=%5d, y=%5d, z=%5d\r\n", imu660rx_gyro_x, imu660rx_gyro_y, imu660rx_gyro_z);
...
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    imu660rx_get_acc();                                                         // 获取 IMU660RX 的加速度测量数值
    imu660rx_get_gyro();                                                        // 获取 IMU660RX 的角速度测量数值
}
...
// 问题3：串口输出 imu660rx init error.或者imu660rx self check error.
//      检查 IMU660RX 的接线是否正确
//      检查 IMU660RX 的模块是不是坏了
//      给信号线加上拉看看
// 问题4：IMU660RX 数值异常
//      看看是不是线松了 或者信号线被短路了
//      可能模块部分受损
