...
// *************************** 例程测试说明 ***************************
// 1.核心板烧录完成本例程 主板电池供电 负压电机接口连接好电机
// 2.如果接了电机 可以看到电机重复由慢到快旋转
// 3.如果没有接电机 使用万用表可以在负压电机接口端子上测量到输出电压变化
...
#define PWM_NEG              (ATOM0_CH0_P21_2)
...
int core0_main(void)
{
    ...
    pwm_init(PWM_NEG, 17000, 0);                                                 // PWM 通道初始化频率 17KHz 占空比初始为 0
	...
	while (TRUE)
	{
        ...
        pwm_set_duty(PWM_NEG, duty * (PWM_DUTY_MAX / 100));                  // 计算占空比
